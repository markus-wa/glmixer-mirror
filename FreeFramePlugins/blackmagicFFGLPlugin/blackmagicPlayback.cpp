...
IDeckLink *getFirstDeckLinkCard()
{
	...
	if (deckLinkIter)
	{
		...
		{
			fprintf(stderr, "BlackMagic : Could not detect a DeckLink card\n");
		}
	...
	{
		fprintf(stderr, "BlackMagic : Could not enumerate DeckLink cards\n");
	}
...
bool	PlaybackHelper::fillFrame(int index)
{
	...
	{
		fprintf(stderr, "BlackMagic : Odd number of pixels in buffer !!!\n");
		goto bail;
	}
	...
	{
		fprintf(stderr, "BlackMagic : Could not get pixel buffer\n");
		goto bail;
	}
...
bool	PlaybackHelper::createFrames()
{
	...
	for(i = 0; i<NUM_FRAMES; i++)
	{
		...
		{
			fprintf(stderr, "BlackMagic : Could not obtain frame %d\n", (i+1));
			goto bail;
		}
...
bool	PlaybackHelper::scheduleNextFrame(bool preroll)
{
	...
	while(iter-- > 0)
	{
		...
		{
			fprintf(stderr, "BlackMagic : Could not schedule next frame (total frame scheduled: %d)\n", m_totalFrameScheduled);
			break;
		}
	}
...
bool	PlaybackHelper::startPlayback()
{
	...
	{
		fprintf(stderr, "BlackMagic : Could not start playback\n");
		goto bail;
	}
	
	fprintf(stderr, "BlackMagic : Playback started\n");
	m_playbackStarted = true;
	
	// Start the watchdog pinging thread
	if (pthread_create(&m_watchdogThread, NULL, pingWatchdogFunc, this) != 0)
	{
		fprintf(stderr, "BlackMagic : Error starting watchdog pinging thread\n");
		goto bail;
	}
...
void	PlaybackHelper::stopPlayback()
{
	...
	fprintf(stderr, "BlackMagic : Playback stopped\n");
...
void	PlaybackHelper::pingWatchdog()
{
	while(m_playbackStarted)
	{
		...
		// {
		// 	fprintf(stderr, "BlackMagic : Error resetting the bypass timeout value\n");
		// 	break;
		// }
...
bool	PlaybackHelper::init()
{
	...
	{
		fprintf(stderr, "BlackMagic : Could not get the IdeckLinkAttributes interface\n");
		goto bail;
	}
	...
	// {
	// 	fprintf(stderr, "BlackMagic : The DeckLink device does not have a bypass\n");
	// 	goto bail;
	// }
	...
	{
		fprintf(stderr, "BlackMagic : Could not get the IDeckLinkConfiguration interface\n");
		m_configuration = NULL;
	}
	...
	{
		fprintf(stderr, "BlackMagic : Could not get DeckLink Output interface\n");
		m_deckLinkOutput = NULL;
		goto bail;
	}
