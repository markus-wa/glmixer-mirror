...
// a QLineEdit that is absolutely identical to default
// EXCEPT it sends a signal with string AFTER editing is finished
class lazyLineEdit : public QLineEdit
{
    Q_OBJECT

public:
    lazyLineEdit(QWidget * parent = 0) : QLineEdit(parent) {
        connect(this, SIGNAL(editingFinished()), SLOT(onEditingFinished()));
    }
    lazyLineEdit(const QString & contents, QWidget * parent = 0) : QLineEdit(contents, parent) {
        connect(this, SIGNAL(editingFinished()), SLOT(onEditingFinished()));
    }
