...
QString View::userInputLabel(View::UserInput ab, bool verbose){
    ...
    if (verbose) {
        switch(ab) {
        case INPUT_ZOOM:
            return QObject::tr("Zoom in (clic source) / zoom out (clic background).");
        case INPUT_CONTEXT_MENU:
            return QObject::tr("Activate the context menu.");
        case INPUT_NAVIGATE:
            return QObject::tr("Navigate in the view.");
        case INPUT_DRAG:
            return QObject::tr("Drag the background (displace all sources).");
        case INPUT_SELECT:
            return QObject::tr("Select / deselect a source.");
        case INPUT_TOOL:
            return QObject::tr("Use the current tool (grab, scale, etc.) on the selected sources.");
        case INPUT_TOOL_INDIVIDUAL:
            return QObject::tr("Use the current tool (grab, scale, etc.) on ONE single source.");
        default:
