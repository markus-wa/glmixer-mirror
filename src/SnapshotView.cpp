...
void SnapshotView::paint()
{
    ...
    static double ax = 0.0, ay = -8.0 * SOURCE_UNIT;
    static double begin = -8.0 * SOURCE_UNIT, end = 8.0 * SOURCE_UNIT;
    ...
    if (!_renderSource) {
        ...
        try {
            ...
            _renderSource->setAlphaCoordinates(begin, ay);
    ...
    glBegin(GL_LINES);
    glVertex3d(begin, ay, 0.0);
    glVertex3d(end, ay, 0.0);
    glEnd();
    glLineWidth(2.0);
    ...
    ax = begin + _factor * (end - begin) ;
    _renderSource->setAlphaCoordinates(ax, ay);
...
bool SnapshotView::mouseMoveEvent(QMouseEvent *event)
{
    ...
    if ( currentAction == View::TOOL ) {
        ...
        grabSource(_renderSource, event->x(), viewport[3] - event->y(), dx, dy);
...
void SnapshotView::grabSource(Source *s, int x, int y, int dx, int dy) {
