...
TimelineSlider::cursor TimelineSlider::mouseOver(QMouseEvent * e)
{
    ...
    else if ( qAbs( getPosFromVal(range_value.first) - e->pos().x()) < GRAB_THRESHOLD ) {

        return CURSOR_RANGE_MIN;
    }
    else if ( qAbs( getPosFromVal(range_value.second) - e->pos().x()) < GRAB_THRESHOLD ) {
...
void TimelineSlider::drawWidget(QPainter &qp)
{
    ...
    QColor COLOR_MARK(palette().color(QPalette::Highlight));
...
//    labelFont.setPointSize(7);
    ...
    while (t < max_value) {
        ...
        // draw single steps if enough space
        ...
        if (step_width > 3) {
            double t2 = t;
            int n = (int) qRound( mark_value / step_value ) - 1;
            while (0 < n && t2 < max_value) {
                t2 += step_value ;
        ...
        // cannot draw single steps, just draw some indicators
        else {
            pos = getPosFromVal(t + mark_value / 2.0);
            qp.drawLine(pos, HEIGHT_TIME_BAR, pos, HEIGHT_TIME_BAR - LINE_MARK_LENGHT / 2);
        }
    ...
    // Draw range
    qp.fillRect(rangeBegin, range_mark_y, rangeEnd - rangeBegin, -RANGE_MARK_HEIGHT, COLOR_RANGE.darker(110));
    ...
    // draw indicators of begin and end
    qp.setPen(PEN_MARK);
    qp.drawPoint(rangeBegin +2, range_mark_y -1);
    qp.drawPoint(rangeEnd -2, range_mark_y -1);

    // compute position begin value
    QString label_b = TimelineSlider::getStringFromTime( begin() );
    int pos_text_b = rangeBegin - metrics.width(label_b) -1;
    if (pos_text_b < 1)
        pos_text_b = rangeBegin + 5;

    // compute position end value
    QString label_e = TimelineSlider::getStringFromTime( end() );
    int pos_text_e = rangeEnd;
    if (pos_text_e + metrics.width(label) > width())
        pos_text_e = rangeEnd - metrics.width(label_e) - 5;
...
void TimelineSlider::updateMarks()
{
    ...
    if ( h > 3 )                // more than 3h
        ...
        mark_value = 10.0;
    else if ( m > 0 )           // between 1 and 2 min
        mark_value = 5.0;
    else if ( s > 30 )          // between 30 and 60 sec
        mark_value = 2.0;
    else if ( s > 15 )          // between 15 and 30 sec
        mark_value = 1.0;
    else if ( s > 5 )           // between 5 and 15 sec
        mark_value = qMax(step_value, 0.5);
    else                        // less than 5 sec
